...
class BaseTester(object):
                ...
                xr = xr.reindex(
                    {
                        "date": pd.DatetimeIndex(
                            pd.date_range(xr["date"].values[0], xr["date"].values[-1], freq=lowest_freq), name="date"
                        )
                    }
                )
            ...
            for freq in results[basins[0]].keys():
                ...
                for i in range(max_figures):
                    ...
                    if target_var in self.cfg.clip_targets_to_zero:
                        sim = xarray.where(sim < 0, 0, sim)
                    figures.append(
                        self._get_plots(obs, sim, title=f"{target_var} - Basin {basins[i]} - Epoch {epoch} - Frequency {freq}")[0]
                    )
        ...
        with torch.no_grad():
            for data in loader:
                data = move_data_to_device(
                    data, self.device
                )  # Custom .to behaviour is needed to account for static_conceptual_params.
    ...
    def _subset_targets(
        self, model: BaseModel, data: Dict[str, torch.Tensor], predictions: np.ndarray, predict_last_n: int, freq: str
    ):
...
class RegressionTester(BaseTester):
    ...
    def _subset_targets(
        self, model: BaseModel, data: Dict[str, torch.Tensor], predictions: np.ndarray, predict_last_n: np.ndarray, freq: str
    ):
...
class UncertaintyTester(BaseTester):
    ...
    def _subset_targets(
        self, model: BaseModel, data: Dict[str, torch.Tensor], predictions: np.ndarray, predict_last_n: int, freq: str = None
    ):
        y_hat_sub = predictions[f"y_hat{freq}"][:, -predict_last_n:, :]
        y_sub = data[f"y{freq}"][:, -predict_last_n:, :]
        return y_hat_sub, y_sub
