...
def initialize_basin_constants(
    ...
    soil_reservoir = {
        ...
        'storage_threshold_secondary_m': soil_config['lateral_flow_threshold_storage_m'], ## but this is the same as field_capacity_storage_threshold_m??
        }
    ...
    N = cfe_params['basinCharacteristics']['giuh_ordinates'].shape[0]
    ...
    flux = {
        'flux_perc_m': torch.tensor(0.0, dtype=torch.float32, device=device).repeat(batch_size)
        }
...
def calculate_evaporation_from_soil(
    ...
    if torch.any(soil_wilting_mask):
        ...
        flux['reduced_potential_et_m_per_timestep'] = flux['reduced_potential_et_m_per_timestep'] - flux['actual_et_from_soil_m_per_timestep']
...
def run_Schaake_subroutine(
    ...
    if torch.any(rainfall_mask):
        ...
        Schaake_parenthetical_term = (1 - torch.exp(- soil_reservoir['Schaake_adjusted_magic_constant_by_soil_type'][soil_deficit_rain_mask] * constants['time']['days']))
        ...
        soil_excess_mask = (flux['timestep_rainfall_input_m'] - flux['infiltration_depth_m'] > 0) # mask for if rainfall is more than infilt depth
        ...
        flux['surface_runoff_depth_m'][combined_soil_excess_mask] = (flux['timestep_rainfall_input_m'] - flux['infiltration_depth_m'])[combined_soil_excess_mask]
        # not written else surface_runoff_depth_m = 0, since initialzied at 0
        flux['infiltration_depth_m'][combined_soil_noExcess_mask] = (flux['timestep_rainfall_input_m'] - flux['surface_runoff_depth_m'])[combined_soil_noExcess_mask]
...
def run_Xinanjiang_subroutine(
    ...
    pervious_runoff_m = torch.where(
        tension_frac_mask,
        (flux['timestep_rainfall_input_m'] * torch.pow(
            (
                0.5 * torch.tensor(1.0, dtype=torch.float32, device=device).repeat(batch_size)
                - a_Xinanjiang_inflection_point_parameter
                ),
    ...
    if torch.any(surface_runoff_rainfall_mask):
        flux['surface_runoff_depth_m'][surface_runoff_rainfall_mask] = flux['timestep_rainfall_input_m'][surface_runoff_rainfall_mask]
...
def run_classic_soil_moisture_subroutine(
    ...
    if torch.any(primary_flux_mask):
        storage_diff_primary = soil_reservoir["storage_max_m"][primary_flux_mask] - soil_reservoir["storage_threshold_primary_m"][primary_flux_mask]
        ...
        flux['primary_flux_m'][primary_flux_mask & primary_above_mask] = storage_above_threshold_primary[primary_flux_mask & primary_above_mask].clone()
    ...
    if torch.any(secondary_flux_mask):
        storage_diff_secondary = soil_reservoir["storage_max_m"][secondary_flux_mask] - soil_reservoir["storage_threshold_secondary_m"][secondary_flux_mask]
        ...
        flux['secondary_flux_m'][secondary_flux_mask & secondary_above_mask] = (storage_above_threshold_secondary - flux['primary_flux_m'])[secondary_flux_mask & secondary_above_mask]
...
def percolation_and_lateral_flow(
    flux: Dict[str, torch.Tensor],
    gw_reservoir: Dict[str, torch.Tensor]
    ) -> Dict[str, torch.Tensor]:
    ...
    if torch.any(no_overflow_mask):
        gw_reservoir["storage_m"][no_overflow_mask] = gw_reservoir["storage_m"][no_overflow_mask] + flux['flux_perc_m'][no_overflow_mask].clone()
...
def soil_reservoir_configuration(
    ...
    H_water_table_m = field_capacity_atm_press_fraction * constants['physics']['atm_press_Pa'] / constants['physics']['unit_weight_water_N_per_m3'] # [m]
    Omega = H_water_table_m - trigger_z_m
    # upper & lower limit of the integral in Eq. 4
    lower_lim = torch.pow(Omega, (1.0 - 1.0 / cfe_params['soil_params']["bb"])) / (
            1.0 - 1.0 / cfe_params['soil_params']["bb"])
    upper_lim = torch.pow(Omega + cfe_params['soil_params']["D"], (1.0 - 1.0 / cfe_params['soil_params']["bb"])) / (1.0 - 1.0 / cfe_params['soil_params']["bb"])
...
def timestep_CFE_new(
    ...
    # from rain
    flux = calculate_evaporation_from_rainfall(
        flux= flux
        )
    # from soil
    flux, soil_reservoir = calculate_evaporation_from_soil(
        flux= flux,
        constants= constants,
        soil_reservoir= soil_reservoir
        )
    ...
    flux, soil_reservoir = adjust_and_track_runoff_infiltration(
        flux= flux,
        soil_reservoir= soil_reservoir
        )
    ...
    if not constants['cfe_scheme']['soil'] in ["classic", "ode"]:
        ...
        flux, soil_reservoir = run_classic_soil_moisture_subroutine(
            flux= flux,
            soil_reservoir= soil_reservoir
        )
    elif constants['cfe_scheme']['soil'] == "ode":
    ...
    flux, soil_reservoir = adjust_from_soil_outflux(
        flux= flux,
        constants= constants,
        soil_reservoir= soil_reservoir
        )
    ...
    flux, gw_reservoir = percolation_and_lateral_flow(
        flux= flux,
        gw_reservoir= gw_reservoir
        )
    ...
    flux, routing_info = calculate_convolutional_integral_for_GIUH(
        flux= flux,
    ...
    flux, routing_info, cfe_params = run_nash_cascade(
        flux= flux,
        routing_info= routing_info,
        cfe_params= cfe_params
        )
