...
from neuralhydrology.utils.dCFE_utils import (
    expand_dcfe_params_along_batch_dim,
    get_dcfe_params,
)
...
class dCFE(BaseConceptualModel):
    ...
    def forward(
        ...
        states, out = self._initialize_information(
            conceptual_inputs=x_conceptual, lstm_out=lstm_out
        )
        ...
        with torch.no_grad():
            for j in range(0, (lstm_out.shape[1] - self.cfg.spin_up)):
                ...
                out[:, j, 0] = (
                    self.flux_Qout_m * 1000
                )  # this will not be included for back prop due to predict_last_n
                # store resulting states, right now this doesn't do anything
                states["gw_reservoir_storage_m"][:, j] = self.gw_reservoir["storage_m"]
                states["soil_reservoir_storage_m"][:, j] = self.soil_reservoir[
                    "storage_m"
                ]
                states["first_nash_storage"][:, j] = self.basinCharacteristics[
                    "nash_storage"
                ][:, 0]
        ...
        for k in range(spin_up_start, lstm_out.shape[1]):
            ...
            states["first_nash_storage"][:, k] = self.basinCharacteristics[
                "nash_storage"
            ][:, 0]
    ...
    def parameter_ranges(self):
        return {
            ...
            "expon": [
                1,
                8,
            ],  # A primary gorundwter nonlinear reservoir exponential constant [-]
    ...
    def timestep_CFE(
        ...
        satdk_timestep: torch.Tensor,
        cgw_timestep: torch.Tensor,
        bb_timestep: torch.Tensor,
        smcmax_timestep: torch.Tensor,
        slop_timestep: torch.Tensor,
        max_gw_timestep: torch.Tensor,
        expon_timestep: torch.Tensor,
        K_lf_timestep: torch.Tensor,
        K_nash_timestep: torch.Tensor,
        ...
        self.timestep_basin_constants(
            satdk_timestep=satdk_timestep,
            cgw_timestep=cgw_timestep,
            bb_timestep=bb_timestep,
            smcmax_timestep=smcmax_timestep,
            slop_timestep=slop_timestep,
            max_gw_timestep=max_gw_timestep,
            expon_timestep=expon_timestep,
            K_lf_timestep=K_lf_timestep,
            K_nash_timestep=K_nash_timestep,
        ...
        if self.schemes["soil"] == "classic":
            ...
            print("Either a classic or ode soil scheme must be chosen.")
            print("Program terminating.:( \n")
    ...
    def initialize_basin_constants(self, x_conceptual: torch.Tensor):
        ...
        self.time_step_size = (
            3600 * 24
        )  # num of [seconds] , we go by 3600s each time step for hourly
        ...
        self.gw_reservoir["storage_m"] = 0.05 * torch.tensor(
        ...
        self.upper_lim = torch.pow(
            self.Omega + self.soil_params["D"], (1.0 - 1.0 / self.soil_params["bb"])
        ) / (1.0 - 1.0 / self.soil_params["bb"])
        # integral & power term in Eq.4, Eq.5
        self.storage_thresh_pow_term = torch.pow(
            1.0 / self.soil_params["satpsi"], (-1.0 / self.soil_params["bb"])
        )
        self.lim_diff = self.upper_lim - self.lower_lim
        self.field_capacity_storage_threshold_m = (
            self.soil_params["smcmax"] * self.storage_thresh_pow_term * self.lim_diff
        )
        ...
        self.soil_reservoir = {
            ...
            * self.soil_params["D"],  # 0.049668*2 = 0.09933
            "storage_max_m": self.soil_params["smcmax"]
            * self.soil_params["D"],  # 0.373*2 = 0.746
            ...
            "coeff_secondary": self.basinCharacteristics[
                "K_lf"
        ...
        self.soil_reservoir["storage_m"] = 0.05 * torch.tensor(
            1.0, dtype=torch.float32, device=x_conceptual.device
        ).repeat(x_conceptual.shape[0])

        self.N = self.basinCharacteristics["giuh_ordinates"].shape[
            0
        ]  # giuh_ordinates are rows x 1 column for each basin, used in routing
        self.runoff_queue_m_per_timestep = torch.ones(
            (x_conceptual.shape[0], self.N + 1),
            dtype=torch.float32,
            device=x_conceptual.device,
        )  # nash cascade
        self.num_reservoirs = self.basinCharacteristics["nash_storage"].shape[
            1
        ]  # 2 reservoirs
        ...
        self.vol = {
            ...
            "partition_runoff": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "partition_infilt": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "et_from_rain": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "et_from_soil": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "et_to_atm": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "to_gw": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "to_soil": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "soil_to_gw": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "soil_to_lat_flow": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "from_gw": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "out_giuh": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "in_nash": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "out_nash": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
            ).repeat(x_conceptual.shape[0]),
            "out": torch.tensor(
                0.0, dtype=torch.float32, device=x_conceptual.device
    ...
    def initialize_basin_constants_hourly(self, x_conceptual: torch.Tensor):
        ...
        # time-related constants
        self.time_step_size = (
            3600  # num of [seconds] , we go by 3600s each time step for hourly
        )
        self.timestep_h = self.time_step_size / 3600  # time step in [hours]
        self.timestep_d = self.timestep_h / 24  # time step in [days]
        # physics constants
    ...
    def timestep_basin_constants(
        ...
        self.basinCharacteristics["K_lf"] = K_lf_timestep
        self.basinCharacteristics["expon"] = expon_timestep

        self.gw_reservoir["storage_max_m"] = self.basinCharacteristics["max_gw_storage"]
        self.gw_reservoir["coeff_primary"] = self.basinCharacteristics["Cgw"]
        self.gw_reservoir["exponent_primary"] = self.basinCharacteristics["expon"]
        ...
        self.soil_reservoir["storage_max_m"] = (
            self.soil_params["smcmax"] * self.soil_params["D"]
        )
        self.soil_reservoir["storage_threshold_primary_m"] = (
            self.field_capacity_storage_threshold_m
        )
        self.soil_reservoir["coeff_secondary"] = self.basinCharacteristics["K_lf"]
        self.soil_reservoir["storage_threshold_secondary_m"] = (
            self.lateral_flow_threshold_storage_m
        )
        self.soil_reservoir["coeff_primary"] = (
            self.soil_params["satdk"] * self.soil_params["slop"] * self.time_step_size
    ...
    def initialize_flux_timestep(self, x_conceptual_timestep: torch.Tensor):
        ...
        self.surface_runoff_depth_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.infilt_excess_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        # infilt excess & surface rnoff depth are the same? I'm using infilt excess to be consistent to cfe
        self.infiltration_depth_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.infilt_depth_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.actual_et_from_rain_m_per_timestep = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.actual_et_from_soil_m_per_timestep = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.actual_et_m_per_timestep = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])  # need to initialize every Epoch?
        # reset ET
        self.reduced_potential_et_m_per_timestep = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])

        self.primary_flux_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.secondary_flux_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        # below are all added later, not in original initialization
        self.primary_flux_from_gw_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.secondary_flux_from_gw_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.flux_giuh_runoff_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.flux_nash_lateral_runoff_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.flux_from_deep_gw_to_chan_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        self.tension_water_m = torch.tensor(
            0.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])  # 'Xinanjiang' partition
    ...
    def get_precip_and_pet(self, x_conceptual_timestep):
        ...
        self.timestep_rainfall_input_m = (
            x_conceptual_timestep[:, 0] / 1000
        )  # convert precip mm/day to [m/day]
        ...
        mean_temp = (
            x_conceptual_timestep[:, 1] + x_conceptual_timestep[:, 2]
        ) / 2  # x_conceptual[:,:,1] is Tmin, x_conceptual[:,:,2] is Tmax
        lambd = 2.501 - 0.002361 * mean_temp
        shortRad = (
            x_conceptual_timestep[:, 3] * self.time_step_size / 1000000
        )  # convert shortwave radiation [W/m^2] to [MJ/m^2 day]
        pet_m_per_timestep_calc = (
            0.025 * shortRad * (mean_temp - (-3.0)) / lambd
        ) / 1000  # convert pet [mm/day] to [m/day]
        pet_m_per_timestep_mask = (
            pet_m_per_timestep_calc < 0
        )  # make mask for negative PET
        self.potential_et_m_per_timestep = torch.where(
    ...
    def get_precip_and_pet_hourly(self, x_conceptual_timestep):
        ...
        self.timestep_rainfall_input_m = (
            x_conceptual_timestep[:, 0] / 1000
        )  # convert precip mm/hr to [m/hr]

        # calculate PET from shortwave rad and mean temp using jensen_evaporation_2016 "https://github.com/pyet-org/pyet/blob/master/pyet/radiation.py"
        lambd = (
            2.501 - 0.002361 * x_conceptual_timestep[:, 1]
        )  # x_conceptual[:,:,1] is mean temp
        shortRad = (
            x_conceptual_timestep[:, 2] * 3600 / 1000000
        )  # convert shortwave radiation [W/m^2] to [MJ/m^2 hr]
        pet_m_per_timestep_calc = (
            0.025 * shortRad * (x_conceptual_timestep[:, 1] - (-3.0)) / lambd
        ) / 1000  # convert pet [mm/hr] to [m/hr]
        pet_m_per_timestep_mask = (
            pet_m_per_timestep_calc < 0
        )  # make mask for negative PET
        self.potential_et_m_per_timestep = torch.where(
    ...
    def calculate_evaporation_from_rainfall(self):
        ...
        if torch.any(rainfall_mask):  # if there's precip in this batch then
            ...
            self.timestep_rainfall_input_m[rainfall_mask] = (
                reduced_rainfall  # adjusting precip based on evaporation from rainfall
            )
            self.reduced_potential_et_m_per_timestep[rainfall_mask] = (
            ...
            self.vol["et_from_rain"] = (
                self.vol["et_from_rain"] + self.actual_et_from_rain_m_per_timestep
            )
            self.vol["et_to_atm"] = (
                self.vol["et_to_atm"] + self.actual_et_from_rain_m_per_timestep
            )
            self.vol["out"] = self.vol["out"] + self.actual_et_from_rain_m_per_timestep
            self.actual_et_m_per_timestep = (
                self.actual_et_m_per_timestep + self.actual_et_from_rain_m_per_timestep
    ...
    def calculate_evaporation_from_soil(self):
        ...
        soil_wilting_mask = (
            self.soil_reservoir["storage_m"] > self.soil_reservoir["wilting_point_m"]
        )

        if torch.any(soil_wilting_mask):
            if self.schemes["soil"] == "classic":
                # Create mask for when reduced PET > 0
                reduced_pet_mask = self.reduced_potential_et_m_per_timestep > 0
                if torch.any(reduced_pet_mask):
                    reduced_pet = self.reduced_potential_et_m_per_timestep[
                        soil_wilting_mask & reduced_pet_mask
                    ]
                    storage_threshold_prim = self.soil_reservoir[
                        "storage_threshold_primary_m"
                    ][soil_wilting_mask & reduced_pet_mask]
                    actual_et_soil = self.actual_et_from_soil_m_per_timestep[
                        soil_wilting_mask & reduced_pet_mask
                    ]
                    soil_storage = self.soil_reservoir["storage_m"][
                        soil_wilting_mask & reduced_pet_mask
                    ]
                    wilting_point = self.soil_reservoir["wilting_point_m"][
                        soil_wilting_mask & reduced_pet_mask
                    ]
                    ...
                    actual_et_soil[storage_threshold_mask] = torch.min(
                        reduced_pet[storage_threshold_mask],
                        soil_storage[storage_threshold_mask],
                    )

                    # for if soil_storage < storage_threshold_prim
                    Budyko_numerator = (
                        soil_storage[~storage_threshold_mask]
                        - wilting_point[~storage_threshold_mask]
                    )
                    Budyko_denominator = (
                        storage_threshold_prim[~storage_threshold_mask]
                        - wilting_point[~storage_threshold_mask]
                    )
                    Budyko = Budyko_numerator / Budyko_denominator
                    actual_et_soil[~storage_threshold_mask] = torch.min(
                        Budyko * reduced_pet[~storage_threshold_mask],
                        soil_storage[~storage_threshold_mask],
                    )
            ...
            self.vol["et_from_soil"] = (
                self.vol["et_from_soil"] + self.actual_et_from_soil_m_per_timestep
            )
            self.vol["et_to_atm"] = (
                self.vol["et_to_atm"] + self.actual_et_from_soil_m_per_timestep
            )
            self.vol["out"] = self.vol["out"] + self.actual_et_from_soil_m_per_timestep
            self.actual_et_m_per_timestep = (
                self.actual_et_m_per_timestep + self.actual_et_from_soil_m_per_timestep
            )
    ...
    def run_Schaake_subroutine(self):
        ...
        self.Schaake_adjusted_magic_constant_by_soil_type = (
            self.basinCharacteristics["refkdt"] * self.soil_params["satdk"] / 2.0e-06
        )
        rainfall_mask = self.timestep_rainfall_input_m > 0
        soil_noDeficit_mask = (
            self.soil_reservoir_storage_deficit_m < 0
        )  # mark ones w/o deficit
        ...
        if torch.any(rainfall_mask):
            ...
            self.surface_runoff_depth_m[soil_noDeficit_rain_mask] = (
                self.timestep_rainfall_input_m[soil_noDeficit_rain_mask]
            )
            ...
            Schaake_parenthetical_term = 1 - torch.exp(
                -self.Schaake_adjusted_magic_constant_by_soil_type[
                    soil_deficit_rain_mask
                ]
                * self.timestep_d
            )
            Ic = (
                self.soil_reservoir_storage_deficit_m[soil_deficit_rain_mask]
                * Schaake_parenthetical_term
            )
            Px = self.timestep_rainfall_input_m[soil_deficit_rain_mask]
            self.infiltration_depth_m[soil_deficit_rain_mask] = Px * (Ic / (Px + Ic))

            # From vector above, make another condition
            soil_excess_mask = (
                self.timestep_rainfall_input_m - self.infiltration_depth_m > 0
            )  # mask for if rainfall is more than infilt depth
            combined_soil_excess_mask = (
                soil_deficit_rain_mask & soil_excess_mask
            )  # mask for above chunk + excess rain
            combined_soil_noExcess_mask = (
                soil_deficit_rain_mask & ~soil_excess_mask
            )  # mask for above chunk + no excess rain
            self.surface_runoff_depth_m[combined_soil_excess_mask] = (
                self.timestep_rainfall_input_m - self.infiltration_depth_m
            )[combined_soil_excess_mask]
            # not written else surface_runoff_depth_m = 0, since initialzied at 0
            self.infiltration_depth_m[combined_soil_noExcess_mask] = (
                self.timestep_rainfall_input_m - self.surface_runoff_depth_m
            )[combined_soil_noExcess_mask]
    ...
    def run_Xinanjiang_subroutine(self, x_conceptual_timestep):
        ...
        free_water_m = (
            self.soil_reservoir["storage_m"]
            - self.soil_reservoir["storage_threshold_primary_m"]
        )  # this storage was adjusted for ET
        ...
        if torch.any(tension_water_mask):
            tension_water_m[tension_water_mask] = max_tension_water_m[
                tension_water_mask
            ]
        """
        ...
        """
        a_Xinanjiang_inflection_point_parameter = torch.tensor(
            1.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        b_Xinanjiang_shape_parameter = torch.tensor(
            1.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
        x_Xinanjiang_shape_parameter = torch.tensor(
            1.0, dtype=torch.float32, device=x_conceptual_timestep.device
        ).repeat(x_conceptual_timestep.shape[0])
                ...
                * torch.pow(
                    (
                        ...
                        * torch.tensor(
                            1.0,
                            dtype=torch.float32,
                            device=x_conceptual_timestep.device,
                        ).repeat(x_conceptual_timestep.shape[0])
                        - a_Xinanjiang_inflection_point_parameter
                    ),
                    (
        ...
        surface_runoff0_mask = (
            self.surface_runoff_depth_m < 0.0
        )  # we have a vector instead of scalar
        ...
        if torch.any(surface_runoff_rainfall_mask):
            self.surface_runoff_depth_m[surface_runoff_rainfall_mask] = (
                self.timestep_rainfall_input_m[surface_runoff_rainfall_mask]
            )
    ...
    def adjust_and_track_runoff_infiltration(self):
        ...
        excess_infil_mask = (
            self.soil_reservoir_storage_deficit_m < self.infiltration_depth_m
        )

        # If there are any such basins, we apply the conditional logic element-wise
        if torch.any(excess_infil_mask):
            diff = (self.infiltration_depth_m - self.soil_reservoir_storage_deficit_m)[
                excess_infil_mask
            ]
            # Adjusting the surface runoff and infiltration depths for the specific basins
            self.surface_runoff_depth_m[excess_infil_mask] = (
                self.surface_runoff_depth_m[excess_infil_mask] + diff
            )
            self.infiltration_depth_m[excess_infil_mask] = (
                self.soil_reservoir_storage_deficit_m[excess_infil_mask]
            )

            # This was missing from original implementation, added by Ziyu 11/11/24 from c code line 142
            self.soil_reservoir["storage_m"][excess_infil_mask] = self.soil_reservoir[
                "storage_max_m"
            ][excess_infil_mask]
        ...
        self.vol["partition_runoff"] = (
            self.vol["partition_runoff"] + self.surface_runoff_depth_m
        )
        self.vol["partition_infilt"] = (
            self.vol["partition_infilt"] + self.infiltration_depth_m
        )
    ...
    def run_classic_soil_moisture_subroutine(self):
        ...
        if torch.any(mask_perc_soil):
            diff_perc_soil = (
                self.flux_perc_m[mask_perc_soil]
                - self.soil_reservoir_storage_deficit_m[mask_perc_soil]
            )
            self.infiltration_depth_m[mask_perc_soil] = (
                self.soil_reservoir_storage_deficit_m[mask_perc_soil]
            )
            #    # not added: lines 170 & 171 send flow back to giuh in self.vol & correct over-prediction of infilt
            self.surface_runoff_depth_m[mask_perc_soil] = (
                self.surface_runoff_depth_m[mask_perc_soil] + diff_perc_soil
            )
        ...
        if torch.any(primary_flux_mask):
            storage_diff_primary = (
                self.soil_reservoir["storage_max_m"][primary_flux_mask]
                - self.soil_reservoir["storage_threshold_primary_m"][primary_flux_mask]
            )
            ...
            storage_power_primary = torch.pow(
                storage_ratio_primary, self.soil_reservoir["exponent_primary"]
            )  # "exponent primary" is scalar for now but can try to init as tensor
            self.primary_flux_m[primary_flux_mask] = (
                self.soil_reservoir["coeff_primary"][primary_flux_mask]
                * storage_power_primary
            )
            # a mask for when primary_flux > storage_above_primary
            primary_above_mask = self.primary_flux_m > storage_above_threshold_primary

            # if primary_flux_m > storage_above_threshold_m then
            self.primary_flux_m[primary_flux_mask & primary_above_mask] = (
                storage_above_threshold_primary[
        ...
        if torch.any(secondary_flux_mask):
            storage_diff_secondary = (
                self.soil_reservoir["storage_max_m"][secondary_flux_mask]
                - self.soil_reservoir["storage_threshold_secondary_m"][
                    secondary_flux_mask
                ]
            )
            storage_ratio_secondary = (
                storage_above_threshold_secondary[secondary_flux_mask]
                / storage_diff_secondary
            )
            storage_power_secondary = torch.pow(
                storage_ratio_secondary, self.soil_reservoir["exponent_secondary"]
            )  # "exponent_secondary" is also a scalar for now
            self.secondary_flux_m[secondary_flux_mask] = (
                self.soil_reservoir["coeff_secondary"][secondary_flux_mask]
                * storage_power_secondary
            )
            # crate a mask for when secondary_flux > storage_above_secondary - primary_flux_m
            secondary_above_mask = self.secondary_flux_m > (
                storage_above_threshold_secondary - self.primary_flux_m
            )

            # if above is true then
            self.secondary_flux_m[secondary_flux_mask & secondary_above_mask] = (
                storage_above_threshold_secondary - self.primary_flux_m
    ...
    def percolation_and_lateral_flow(self):
        ...
        gw_reservoir_storage_deficit_m = (
            self.gw_reservoir["storage_max_m"] - self.gw_reservoir["storage_m"]
        )
        ...
        if torch.any(overflow_mask):
            ...
            diff = (self.flux_perc_m - gw_reservoir_storage_deficit_m)[
                overflow_mask
            ].clone()
            ...
            self.surface_runoff_depth_m[overflow_mask] = (
                self.surface_runoff_depth_m[overflow_mask] + diff
            )

            # Reduce the infiltration (maximum possible flux_perc_m is equal to gw_reservoir_storage_deficit_m)
            self.flux_perc_m[overflow_mask] = gw_reservoir_storage_deficit_m[
                overflow_mask
            ].clone()

            # Saturate the Groundwater storage # I believe storage + flux_prec_m saturated = storage max
            self.gw_reservoir["storage_m"][overflow_mask] = self.gw_reservoir[
                "storage_max_m"
            ][overflow_mask].clone()
            ...
            # Track volume
            self.vol["partition_runoff"][overflow_mask] = (
                self.vol["partition_runoff"][overflow_mask] + diff
            )
            self.vol["partition_infilt"][overflow_mask] = (
                self.vol["partition_infilt"][overflow_mask] + diff
            )
        ...
        if torch.any(no_overflow_mask):
            self.gw_reservoir["storage_m"][no_overflow_mask] = (
                self.gw_reservoir["storage_m"][no_overflow_mask]
                + self.flux_perc_m[no_overflow_mask].clone()
            )
    ...
    def calculate_gw_reservoir_flux(self, x_conceptual_timestep: torch.Tensor):
        ...
        flux_exponential = torch.exp(
            self.gw_reservoir["exponent_primary"]
            * self.gw_reservoir["storage_m"]
            / self.gw_reservoir["storage_max_m"]
        ) - torch.ones(
            (x_conceptual_timestep.shape[0]),
            dtype=torch.float32,
            device=x_conceptual_timestep.device,
        )
        ...
        self.flux_from_deep_gw_to_chan_m = (
            self.primary_flux_from_gw_m + self.secondary_flux_from_gw_m
        )  # there's no 2nd flux since exponential

        ### track_volume_from_gw
        self.gw_reservoir["storage_m"] = (
            self.gw_reservoir["storage_m"] - self.flux_from_deep_gw_to_chan_m.clone()
        )
    ...
    def calculate_convolutional_integral_for_GIUH(self):
        ...
        self.runoff_queue_m_per_timestep[:, :-1] = self.runoff_queue_m_per_timestep[
            :, :-1
        ] + (
            self.basinCharacteristics["giuh_ordinates"]
            * self.surface_runoff_depth_m.expand(self.N, -1).T
        )
    ...
    def run_nash_cascade(self):
        ...
        Q = (
            self.basinCharacteristics["K_nash"].unsqueeze(1) * nash_storage_timestep
        )  # first pass would be 0
    ...
    def testCFE_Daily(
        self, x_conceptual: torch.Tensor, parameters: torch.Tensor
    ) -> torch.Tensor:
        ...
        config_path = Path(
            "/Users/ziyu/Library/CloudStorage/OneDrive-ColoradoSchoolofMines/Documents/College/ResearchStuff/NextGen/neuralhydrology/examples/07-DifferentialCFE-Model/basin_dCFEwPETHourly.yml"
        )
    ...
    def testCFE_Hourly(
        ...
        config_path = Path(
            "/Users/ziyu/Library/CloudStorage/OneDrive-ColoradoSchoolofMines/Documents/College/ResearchStuff/NextGen/neuralhydrology/examples/07-DifferentialCFE-Model/basin_dCFEwPETDaily.yml"
        )
        cfg = Config(config_path, dev_mode=True)
