...
class testCFE:
    ...
    Last edited: by Ziyu, 2025-02-12
    
    This is a test version to CFE output from https://github.com/NWC-CUAHSI-Summer-Institute/cfe_py/blob/main/cat58_test_compare.csv
    ...
    def testRun(self, x_conceptual: torch.Tensor) -> torch.Tensor:
        ...
        Discharge = torch.zeros(x_conceptual.shape[1], 8)
        
        self.basinCharacteristics = {
            'catchment_area_km2': 15.617167355002097 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]), 
            'refkdt': 3.8266861353378374 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]),
            'max_gw_storage': 16 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]),
            'Cgw': 0.01*torch.tensor(1.0).repeat(x_conceptual.shape[0]),
            'expon': 6.0 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]),
            ...
            'giuh_ordinates': torch.tensor([0.1, 0.35, 0.2, 0.14, 0.1, 0.06, 0.05], dtype=torch.float32, device=x_conceptual.device)
        } 
        ...
        self.soil_params = {
            ...
            'bb': 4.05 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]), # exponent on Clapp-Hornberger function, part of calibration
            'satdk': 0.00000338*torch.tensor(1.0).repeat(x_conceptual.shape[0]),
            'satpsi': 0.355 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]), 
            'slop': 1 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]), # slope coefficient, part of calibration
            'smcmax': 0.439 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]), # maximum soil moisture content [m3/m3], part of calibration
            'wltsmc': 0.066* torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]),
        ...
        for i in range(x_conceptual.shape[1]):
            self.timestep_CFE(x_conceptual_timestep = x_conceptual[:,i,:],
                                  satdk_timestep = self.soil_params['satdk'],
                                  cgw_timestep = self.basinCharacteristics['Cgw'],
                                  bb_timestep = self.soil_params['bb'],
                                  smcmax_timestep = self.soil_params['smcmax'],
                                  slop_timestep = self.soil_params['slop'],
                                  max_gw_timestep = self.basinCharacteristics['max_gw_storage'],
                                  expon_timestep = self.basinCharacteristics['expon'],
                                  K_lf_timestep = self.basinCharacteristics['K_lf'],
                                  K_nash_timestep = self.basinCharacteristics['K_nash'])
    ...
    def parameter_ranges(self):
        return {'satdk': [0.0, 0.000726], # Saturated hydraulic conductivity [m/hr]
    ...
    def initialize_basin_constants(self, x_conceptual: torch.Tensor):
        ...
        self.gw_reservoir['storage_m'] = self.gw_reservoir['storage_max_m'].clone() * 0.03125
        ...
        self.soil_reservoir['storage_m'] = 0.667 * torch.tensor(1.0, dtype=torch.float32, device=x_conceptual.device).repeat(x_conceptual.shape[0]) #factor was 0.6 before
    ...
    def timestep_basin_constants(self, satdk_timestep: torch.Tensor, cgw_timestep: torch.Tensor, bb_timestep: torch.Tensor, 
        ...
        self.soil_reservoir['storage_max_m'] = self.soil_params['smcmax']*self.soil_params['D']
        self.soil_reservoir['storage_threshold_primary_m'] = self.field_capacity_storage_threshold_m
        self.soil_reservoir['coeff_secondary'] = self.basinCharacteristics['K_lf']
        self.soil_reservoir['storage_threshold_secondary_m'] = self.lateral_flow_threshold_storage_m
    ...
    def initialize_flux_timestep(self, x_conceptual_timestep: torch.Tensor):
        ...
        self.infilt_excess_m = torch.tensor(0.0, dtype=torch.float32, device=x_conceptual_timestep.device).repeat(x_conceptual_timestep.shape[0])
        ...
        self.infilt_depth_m = torch.tensor(0.0, dtype=torch.float32, device=x_conceptual_timestep.device).repeat(x_conceptual_timestep.shape[0])
    ...
    def get_precip_and_pet(self, x_conceptual_timestep):
        ...
        self.timestep_rainfall_input_m = x_conceptual_timestep[:,0]*self.time_step_size # convert precip mm/day to [m/day]
        ...
        mean_temp = x_conceptual_timestep[:,1] - 273.15 
        lambd = 2.501 - 0.002361 * mean_temp
        shortRad = x_conceptual_timestep[:,2]*self.time_step_size/1000000 # convert shortwave radiation [W/m^2] to [MJ/m^2 day]
        ...
        if torch.any(soil_wilting_mask):
            if self.schemes['soil'] == "classic":
                ...
                if torch.any(reduced_pet_mask):
                    reduced_pet = self.reduced_potential_et_m_per_timestep[soil_wilting_mask & reduced_pet_mask]
                    storage_threshold_prim = self.soil_reservoir["storage_threshold_primary_m"][soil_wilting_mask & reduced_pet_mask]
                    actual_et_soil = self.actual_et_from_soil_m_per_timestep[soil_wilting_mask & reduced_pet_mask]
                    soil_storage = self.soil_reservoir["storage_m"][soil_wilting_mask & reduced_pet_mask]
                    wilting_point = self.soil_reservoir["wilting_point_m"][soil_wilting_mask & reduced_pet_mask]
    ...
    def run_Schaake_subroutine(self):
        ...
        if torch.any(rainfall_mask):
            ...
            Schaake_parenthetical_term = (1 - torch.exp(- self.Schaake_adjusted_magic_constant_by_soil_type[soil_deficit_rain_mask] * self.timestep_d))
            Ic = self.soil_reservoir_storage_deficit_m[soil_deficit_rain_mask] * Schaake_parenthetical_term
    ...
    def run_classic_soil_moisture_subroutine(self):
        ...
        #    # not added: lines 170 & 171 send flow back to giuh in self.vol & correct over-prediction of infilt
            self.surface_runoff_depth_m[mask_perc_soil] = self.surface_runoff_depth_m[mask_perc_soil] + diff_perc_soil
        ...
        if torch.any(primary_flux_mask):
            storage_diff_primary = self.soil_reservoir["storage_max_m"][primary_flux_mask] - self.soil_reservoir["storage_threshold_primary_m"][primary_flux_mask]
            ...
            self.primary_flux_m[primary_flux_mask] = self.soil_reservoir["coeff_primary"][primary_flux_mask] * storage_power_primary
            ...
            self.primary_flux_m[primary_flux_mask & primary_above_mask] = storage_above_threshold_primary[primary_flux_mask & primary_above_mask].clone()
        ...
        if torch.any(secondary_flux_mask):
            storage_diff_secondary = self.soil_reservoir["storage_max_m"][secondary_flux_mask] - self.soil_reservoir["storage_threshold_secondary_m"][secondary_flux_mask]
            ...
            self.secondary_flux_m[secondary_flux_mask] = self.soil_reservoir["coeff_secondary"][secondary_flux_mask] * storage_power_secondary
            # crate a mask for when secondary_flux > storage_above_secondary - primary_flux_m
            secondary_above_mask = self.secondary_flux_m > (storage_above_threshold_secondary - self.primary_flux_m)
            
            # if above is true then
            self.secondary_flux_m[secondary_flux_mask & secondary_above_mask] = (storage_above_threshold_secondary - self.primary_flux_m)[secondary_flux_mask & secondary_above_mask]
